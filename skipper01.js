...
// @include /^(https?:\/\/)(tnlink|jrlinks|shortzzy|coincity|techmart4u|9bitco|qualitystudymaterial|adoco|cuturl|missionhight|wpking|foodma|zagl|ls2earn|streamshort|cslink|coinbaze|blog.earn4fun|blog.earn2fly|shareus|referus|megafly|megaurl|cutp|sureshjonna|earnmoj|riadshot|urlmoney|cplink|cryptomonitor|technoflip|linkshortify|studyranks|htlinks|techlearningtube|manthans|urlfly|linkocean|financeflix|uttan|jazbaat|short.faucetcrypto|smallshort|djxmaza|bildirim|techy4you|battlechamp|sahilsumra|kerdlinfo|urlcut|insurancevlog|itechlogic|tellchimp|videolyrics|tekcrypt|du-link|moddingzone|adybrands|gpflix|gz2)(\.in)(\/.*)/
// @include /^(https?:\/\/)(.+)?(cashurl|linkad|bitcoinly|crazyblog|zolomix|technologylover|earnfort|spidermods|expertlinks|linksfire|largestpanel|linkres|rsrlink|linkstream|xmod|short2url|99links|rplinks|enit|adloot|url2go|tnvalue|hpshort|hblinks|pvidly|apurl|linkshor|earnspace|urlinked|sniply|mdlink|gamerfang|writedroid)(\.in)(\/.*)?/
// @include /^(https?:\/\/)(aylink|linkszia|claimsatoshi|mynewsmedia|bitshort|gtlink|earnload|adfloz|linksly|droplink|123link|go.leolink|techload|linksfy|adshorti|hxfile|dosya|veganal|takem|ezimg|eng.ezimg|dausel|9-animie|veganho|earn24|nulledmod|veganac)(\.co)(\/.*)/
// @include /^(https?:\/\/)(.+)?(takez|linksfire|vosan|tinygo)(\.co)(\/.*)/
// @include /^(https?:\/\/)(ctbl|skyurl|girls-like|adsoro|gobits|zoss|shrinke|tlin|makeshort|terafly|petafly|gigafly|adnews|papanews|panyflay|mozzfly|best-news|mega-news|health-and|short.croclix|stfly|adbull|cutx|youshort|wildblog|specx|dddrive|linkpoi|sh24|m.newhit|mboost|linkshorts|exafly|nonofly|weezo|lozzfly|uptomega)(\.me)(\/.*)/
// @include /^(https?:\/\/)(.+)?(theconomy|adcorto|adsurfing|richlink|wildlinks)(\.me)(\/.*)/
// @include /^(https?:\/\/)(sitr|palpodcast|safelink|zipcrypto|cutgo|cutbits|shorthero|flylink|earncoin|downfile|savelink|linkshortify|url.coinfree|shareus|zo.coin-city|promo-visits|mobileprice)(\.site)(\/.*)/
// @include /^(https?:\/\/)(.+)?(bshopme|cekip|kolotoken|link4rev)(\.site)(\/.*)/
// @include /^(https?:\/\/)(earnme|sanoybonito|forexat|automotur|1ist|kooza|atlai|claimbitco|1link|zentum|9btc|ethereumearner4|doubledown|mytop5|forexwaw)(\.club)(\/.*)/
...
// @include /^(https?:\/\/)(url.faucet77|bloogerspoot|ashort2a-bro|link100s|linkshor|faucetworld)(\.tk)(\/.*)/
...
// @include /^(https?:\/\/)(coinshub|myhealths|kiiw|passgen|wordcounter|shrink|revcot)(\.icu)(\/.*)/
// @include /^(https?:\/\/)(.+)?(flyzu)(\.icu)(\/.*)/
// @include /^(https?:\/\/)(luckybits|newforex|uebnews|wplink|nbyts|wealthystyle|ls2earn|happy-living|earn-cash|yourtechnology|bell.healthyguy|apks|getstudy|network-earn|teacherana|earnow)(\.online)(\/.*)/
// @include /^(https?:\/\/)(.+)?(adslinkfly|readi)(\.online)(\/.*)/
// @include /^(https?:\/\/)(exey|ezlinks|techmody|ouo|ex-e|short.goldenfaucet|saly|cuty|evoload|bitfly|sh.swissfaucet|sl.swissfaucet|oke|letsupload|eio|oii)(\.io)(\/.*)/
...
// @include /^(https?:\/\/)(linkmit|zcpa|jameeltips|mitly|adfoc|link4fun|adsly)(\.us)(\/.*)/
...
// @include /^(https?:\/\/)(profitlink|zagl|scratch247|siyn|portablesusb|children-learningreading|azmath|mobilereview|cryptofly|inform3tion|mobilism|machicon-akihabara|cooklike|videoclip|bitcrypto|shortlinkto|uptobhai|brbushare|fanclup)(\.info)(\/.*)/
// @include /^(https?:\/\/)(hcsbtc|alertcrypto|aboutprofit|faucetcrypto|bildirim)(\.eu)(\/.*)/
// @include /^(https?:\/\/)(antonimos|blog.hostratgeber)(\.de)(\/.*)/
// @include /^(https?:\/\/)(freebitcoin|freelitecoin|uplinkto)(\.vip)(\/.*)/
...
// @include /^(https?:\/\/)(.+)?(exe|exee|exep)(\.app)(\/.*)/
...
(function() {
    ...
    case 'dogeearn.com': case 'mcrypto.club': case 'adarima.org': if (/^\?([^&]+)/.test(h.search)) { location.href = 'https://adarima.org/author?/' + decodeURIComponent(RegExp.$1);} break;
    ...
    if (['interactive', 'complete'].includes(document.readyState)) {onHtmlLoaded();} else {document.addEventListener('DOMContentLoaded', onHtmlLoaded);} function onHtmlLoaded() {if (['fast.nurul-huda.or.id', 'rotator.nurul-huda.sch.id'].indexOf(location.host) > -1){ } else { const listhide = document.querySelectorAll('#frame,.adb,#iframe,#particles-js,#canvas,.floawing-banner,#orquidea-slideup,.floating-banner,#wcfloatDiv4,#preloader,.vanta-canvas,.particles-js-canvas-el,#s65c,#ofc9,#overlay,#footerads,#d7b2,#_m_a_,#masthead_,.oulcsy,.sticky-bottom,.sticky-bottom1,.sticky-bottom2,.sticky-bottom3,.bElNbEFRL3JvbG89,.separator,html body iframe,html iframe,#cookie-pop,.vmyor,.tnjmvneldp,.full_post_area.rh-container,.sc-content-aside,#ofc9,.floawing-banner');
    ...
    // setInterval Mode
    // ============================================
    clickIfElementExists('#slu-link', 3, 'setInterval');
    ...
    if (location.host === 'teknosimple.com' && bp('.slu-captcha') !== null) {var ts = function() {if (window.grecaptcha.getResponse?.().length > 0) {bp('#slu-continue.slu-button').click(); clearInterval(k);}};var ks = setInterval(ts, 500);}
    if (['nightfallnews.com','fc-lc.com', 'chooyomi.com','zipfy.link','paid4.link','usersdrive.com', 'zentum.club', 'patak.dropz.xyz', 'workupload.com', 'taiwancnan.com','solarchaine.com','shortit.pw'].indexOf(location.host) > -1) {window.top != window.self;} else {clickIfElementExists('button.btn:nth-child(4)', 2, 'setInterval');}
    ...
    let $ = window.jQuery; let respect = 'https://fast.nurul-huda.or.id/?url='; // Don't use My Scripts if You Change/Remove My Blogs, Except You Make Donations!!!.
    ...
    switch (h.host) {case 'fast.nurul-huda.or.id': case 'rotator.nurul-huda.sch.id': if (h.pathname === '/' && h.searchParams.has('url') && h.searchParams.has('sn')) {
    ...
    case 'ls2earn.in': if ( h.searchParams.has('url') && h.href.includes('bloogerspoot.') ) {window.location.assign('https://softindex.website'+ (location.search).substring(130));} break;
    case 'ls2earn.com': case 'besturl.link': if ( h.searchParams.has('url') && h.href.includes('bloogerspoot.') ) {window.location.assign('https://softindex.website'+ (location.search).substring(144));} break;
    ...
    BloggerPemula('tr.link|fauceteen.xyz', 'url,api', '', false);
    ...
    BloggerPemula('glamgreet.com|autodime.com', 'go', 'https://go.linkrex.net/', false);
    ...
    BloggerPemula('kienthucrangmieng.com|coindoog.com|tieutietkiem.com|askpaccosi.com|shopsixseven.com|hoclamdaubep.com|thuocdangian.net|chinhnhacoban.com|coin-free.com|fanclup.info', 'wpsafelink', '', false);
    ...
    BypassedByBloggerPemula(/btcpany.com|zombiebtc.com/, function() { bp('#link1s-form').submit();});
    ...
    BypassedByBloggerPemula(/mobitaak.com|forex-gold.net/, function() {clickIfElementExists('.s-btn-f', 5, 'setInterval');});
    ...
    BypassedByBloggerPemula(/bitfly.io|newforex.online|forex-gold.net|world-trips.net|mobi2c.com/, function() {clickIfElementExists('#submitbtn', 3, 'setInterval');});
    ...
    BypassedByBloggerPemula(/linkvertise.com|linkvertise.download/, function() {var linkver = new XMLHttpRequest; linkver.responseType = 'json'; linkver.open('GET', `https://main-bypass-server.tk/v2?${document.location.href.slice(0,-1)}`, !0); linkver.onload = function(n) {4 === linkver.readyState && redirect(this.response.destination);}; linkver.send(null);});
    ...
    BypassedByBloggerPemula(/sub2unlock.com/, function() {if (elementExists('.locked-button')) {fetch(`https://api.bypass.vip/`,{"headers":{"content-type":"application/x-www-form-urlencoded; charset=UTF-8","x-requested-with":"XMLHttpRequest"},"body":new URLSearchParams({url:location.href}),"method":"POST",}).then(res=>res.json()).then(res=>{if(res.status!=="success"){redirect(res.destination);}});}else {}});
    BypassedByBloggerPemula(/faucet.work|techlearningtube.in|wildblog.me|bebodybuilder.com|blogginglass.com|technoflip.in|studyranks.in|brixarena.tech|crypto-blog.xyz|jiotech.net|saqercoin.xyz|crypto4tun.com|mrsaifi.xyz|manthans.in|adshorturl.com|digitalnaz.net|welovecrypto.xyz|arahtekno.com|mopahealth.com/, function() {var el = document.querySelector("input[name=newwpsafelink]");redirect(JSON.parse(atob(el.value)).linkr);});
    BypassedByBloggerPemula(/diudemy.com|freegamestops.com|maqal360.com|appsbull.com/, () =>{function strBetween(s,a,b) { return s.slice(s.indexOf(a)+a.length,s.indexOf(b));}window.setTimeout(()=>{if ( bp(`#linkNext`) && bp(`body > script:nth-child(22)`).innerHTML.includes(`function time() {`)) {let str = bp(`body > script:nth-child(22)`).innerHTML;let xAction = strBetween(str,`form action="`,`/$`);let xRandom = strBetween(str,`" class="text-center `,`" method="POST"> <`);
    let xHashed = strBetween(str,`name="hashed" value="`,`"> <${xRandom}`);let xLinkNx = bp(`#linkNext`).value; if (xHashed != "") {let btn = `<form action="${xAction}/${xLinkNx}" class="text-center ${xRandom}" method="POST"><input type="hidden" name="hashed" value="${xHashed}"><${xRandom} id="next" onClick="${xRandom} ()" name="submit" class="blog_btn next_btn" >please Wait...</form>`;console.log(btn);bp(`#appended`).innerHTML = btn;bp(`.${xRandom}`).submit();} else console.log(`empty hashed value.`);bp(`a#next`).click();}}, 3500);});
    BypassedByBloggerPemula(/99faucet.com|readi.online|cempakajaya.com|tribuncrypto.com|bitzite.com|intmagz.com|nivaprofit.xyz|videoclip.info|freebitcoin.fun|blog-forall.com|punyablog.my.id|punyamerk.top|lycoslink.com|aduzz.com|soltoshindo.com|crypto-news-hub.com|onlineteori.my.id|xtrabits.click|onlineincoms.com|crypto-fi.net|claimcrypto.cc|ourcoincash.xyz|9-animie.co|sapica.xyz|techleets.xyz|moddingzone.in|chuksblog.fun|liongiv.xyz|bioinflu.com/, function(){var bypasslink = atob(`aH${bp("#landing [name='go']").value.split("aH").slice(1).join("aH")}`); redirect(bypasslink, false);});
    BypassedByBloggerPemula(/patak.dropz.xyz/, function() {if (window.location.href.includes('/captcha')&& !window.location.href.includes('/roll')) {}else if (window.location.href.includes('/site-friends')) {let windowName = "";let currentwindowOpenFunction = unsafeWindow.open;function newwindowOpenFunction(params1, params2) {if (!params2 || params2 == "_blank") {windowName = "popUpWindowMyDropz";} else {windowName = params2;} return currentwindowOpenFunction(params1, windowName);};unsafeWindow.open = newwindowOpenFunction;unsafeWindow.onbeforeunload = function() {currentwindowOpenFunction('', windowName).close();};
    ...
    for(let hc=0; hc < document.querySelectorAll("iframe").length; hc++){if(!clicked && document.querySelectorAll("iframe")[hc] && document.querySelectorAll("iframe")[hc].hasAttribute("data-hcaptcha-response") && document.querySelectorAll("iframe")[hc].getAttribute("data-hcaptcha-response").length > 0) {for(let i=0; i< document.querySelectorAll(".btn.btn-info").length; i++){if(!clicked && !endsWithNumber(document.querySelectorAll(".btn.btn-info")[i].getAttribute('id')) && document.querySelectorAll(".btn.btn-info")[i].innerText == "Submits"){document.querySelectorAll(".btn.btn-info")[i].click(); clicked = true; break;}}}}},10000);} else {}});
